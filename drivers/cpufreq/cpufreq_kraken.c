...
{
	if (p->cur == p->max) {
		trace_cpufreq_kraken_already (p->cpu, load, p->cur, p->cur, p->cur);
		return;
	}

	trace_cpufreq_kraken_target (p->cpu, load, p->cur, p->cur, freq);

	__cpufreq_driver_target(p, freq, (freq < p->max) ?
			CPUFREQ_RELATION_L : CPUFREQ_RELATION_H);

	trace_cpufreq_kraken_up (p->cpu, freq, p->cur);
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	{
		trace_cpufreq_kraken_already (policy->cpu, cur_load, policy->cur, policy->cur, policy->cur);
		return;
	}
	...
	if (max_load_freq <
		...
		trace_cpufreq_kraken_target (policy->cpu, cur_load, policy->cur, policy->cur, freq_next);
		__cpufreq_driver_target(policy, freq_next,
			CPUFREQ_RELATION_L);
		trace_cpufreq_kraken_down (policy->cpu, freq_next, policy->cur);
	}
}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		trace_cpufreq_kraken_target (cpu, 0, 0, 0, 0);
		break;
